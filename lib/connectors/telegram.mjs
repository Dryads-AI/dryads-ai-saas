...
export class TelegramConnector extends BaseConnector {
        ...
        if (data.ok && Array.isArray(data.result)) {
          for (const update of data.result) {
            ...
            if (msg.voice || msg.audio) {
              ...
              console.log(`[TG:Voice] From ${msg.from?.first_name || "User"} (${chatId}): voice (${audio.duration || "?"}s)`)
              this._sendTyping(chatId)
              this._processVoiceMessage(msg, audio).catch((err) => {
                console.error("[TG:Voice] Error:", err.message)
                this.send(chatId, "Sorry, I couldn't understand the voice message. Please try again or type your message.").catch(() => {})
              })
              continue
            }
  ...
  async _processVoiceMessage(msg, audio) {
    const chatId = String(msg.chat.id)

    const openaiKey = await getOpenAIKeyForUser(this.pool, this.userId)
    if (!openaiKey) {
      await this.send(chatId, "⚙️ Voice transcription requires an OpenAI API key. Add one in Settings.")
      return
    }

    // Get file path from Telegram
    const fileRes = await fetch(this._url("getFile"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ file_id: audio.file_id }),
    })
    const fileData = await fileRes.json()
    if (!fileData.ok) throw new Error(`getFile failed: ${fileData.description}`)

    const fileUrl = `https://api.telegram.org/file/bot${this._token}/${fileData.result.file_path}`
    const ext = fileData.result.file_path?.split(".").pop() || "ogg"

    console.log(`[TG:Voice] Downloading and transcribing...`)
    const sttResult = await transcribeFromUrl(openaiKey, fileUrl, { filename: `voice.${ext}` })

    if (!sttResult.text.trim()) {
      console.log("[TG:Voice] Transcription empty — skipping")
      return
    }

    console.log(`[TG:Voice] Transcribed: "${sttResult.text.slice(0, 80)}"`)
    // Process as a regular text message
    await this._processMessage({ ...msg, text: sttResult.text, _isVoiceNote: true })
  }
