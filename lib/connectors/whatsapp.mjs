/**
 * Dryads AI — WhatsApp Connector (Baileys — QR Code Scan)
 * Personal mode: QR code scanning via Baileys (WhatsApp Web protocol).
 * Uses PostgreSQL auth state adapter for ephemeral hosting.
 */

import { makeWASocket, DisconnectReason, fetchLatestBaileysVersion, Browsers, downloadMediaMessage } from "@whiskeysockets/baileys"
import { Boom } from "@hapi/boom"
import { SocksProxyAgent } from "socks-proxy-agent"
import { usePgAuthState } from "../baileys-auth-pg.mjs"
import { BaseConnector } from "./base.mjs"
import { transcribeAudio, getOpenAIKeyForUser } from "../ai/stt.mjs"
...
export class WhatsAppConnector extends BaseConnector {
  ...
  async sendImage(jid, imageUrl, caption) {
    if (!this._sock) throw new Error("WhatsApp not connected")
    try {
      // Download image as buffer first (DALL-E URLs are temporary and expire)
      console.log(`[WA] Downloading image: ${imageUrl.slice(0, 80)}...`)
    ...
    try {
      ...
      sock.ev.on("messages.upsert", async (upsert) => {
        ...
        for (const msg of msgs) {
          ...
          // Detect voice messages (audio/ptt)
          const audioMsg = msg.message?.audioMessage
          const isVoiceNote = audioMsg && (audioMsg.ptt === true || audioMsg.mimetype?.includes("audio"))

          if (!text.trim() && !isVoiceNote) continue

          const jid = msg.key.remoteJid
          const pushName = msg.pushName || "User"
          const peer = jid.replace(/@s\.whatsapp\.net$/, "")

          // STT: Transcribe voice notes to text
          if (isVoiceNote) {
            console.log(`[WA:Voice] From ${pushName} (${peer}): voice note (${audioMsg.seconds || "?"}s, ${audioMsg.mimetype})`)
            try {
              // Get OpenAI key for Whisper transcription
              const openaiKey = await getOpenAIKeyForUser(this.pool, this.userId)
              if (!openaiKey) {
                console.error("[WA:Voice] No OpenAI key for STT — skipping voice note")
                await sock.sendMessage(jid, { text: "⚙️ Voice transcription requires an OpenAI API key. Add one in Settings." }).catch(() => {})
                continue
              }
              ...
              if (!audioBuffer || audioBuffer.length === 0) {
                console.error("[WA:Voice] Failed to download audio")
                continue
              }

              console.log(`[WA:Voice] Downloaded ${audioBuffer.length} bytes, transcribing...`)

              // Transcribe with OpenAI Whisper
              const sttResult = await transcribeAudio(openaiKey, audioBuffer, {
                filename: "voice.ogg",
              })

              text = sttResult.text
              if (!text.trim()) {
                console.log("[WA:Voice] Transcription empty — skipping")
                continue
              }

              console.log(`[WA:Voice] Transcribed: "${text.slice(0, 80)}"`)
            } catch (err) {
              console.error("[WA:Voice] STT error:", err.message)
              await sock.sendMessage(jid, { text: "Sorry, I couldn't understand the voice message. Please try again or type your message." }).catch(() => {})
              continue
            }
          }
