...
export class ConnectorRegistry {
  ...
  async startConnector(userId, channelType, connectionMode, config) {
    ...
    if (!ConnectorClass) {
      // Generic/unsupported channel — log once and remember
    ...
    for (const row of res.rows) {
      ...
      if (!this.connectors.has(key)) {
        if (this._failedKeys.has(key)) {
          // Only log once per key
          if (!this._failedLogged) this._failedLogged = new Set()
          if (!this._failedLogged.has(key)) {
            this._failedLogged.add(key)
            console.log(`[Registry] Skipping ${row.channelType}(${connectionMode}) — previously failed`)
          }
        } else {
