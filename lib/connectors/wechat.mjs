    ...
    try {
      ...
      bot.on("message", async (message) => {
        ...
        if (type === Message.Type.Audio) {
          ...
          console.log(`[WeChat:Voice] From ${talker.name()}: voice message`)
          try {
            const openaiKey = await getOpenAIKeyForUser(this.pool, this.userId)
            if (!openaiKey) {
              await talker.say("⚙️ Voice transcription requires an OpenAI API key. Add one in Settings.").catch(() => {})
              return
            }
            const fileBox = await message.toFileBox()
            const buffer = await fileBox.toBuffer()
            if (!buffer || !buffer.length) {
              console.log("[WeChat:Voice] Empty audio — skipping")
              return
            }
            const filename = fileBox.name || "voice.silk"
            const sttResult = await transcribeAudio(openaiKey, buffer, { filename })
            if (!sttResult.text.trim()) {
              console.log("[WeChat:Voice] Transcription empty — skipping")
              return
            }
            text = sttResult.text
            console.log(`[WeChat:Voice] Transcribed: "${text.slice(0, 80)}"`)
          } catch (err) {
            console.error("[WeChat:Voice] STT error:", err.message)
            await message.talker().say("Sorry, I couldn't understand the voice message.").catch(() => {})
            return
          }
