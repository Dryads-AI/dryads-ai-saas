  ...
  async (args, ctx) => {
    ...
    try {
      ...
      if (res.rows.length === 0) {
        ...
        const tips = isPhoneQuery
          ? `No contacts found matching number "${query}". This person may not have messaged the bot yet. ` +
            `However, you CAN still send them a message directly using send_message with the phone number as the peer_id.`
          : `No contacts found matching "${query}". The person may not have chatted with the bot yet, or the name might be spelled differently.`

        return JSON.stringify({
          status: "no_results",
          query,
          is_phone: isPhoneQuery,
          digits: isPhoneQuery ? digitsOnly : undefined,
          message: tips,
        })
      }
